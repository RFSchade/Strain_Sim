...
Sprint <- function(len, df){
  # Advancing in the sprint
  df$sprint_turn <- df$sprint_turn + 1
  
  # This happens at the last turn of the sprint
  if(df$sprint_turn == len){
    # agent_specs moves closer to project_specs if agent_specs is greater  
    if(df$agent_specs > df$project_specs){
      df$agent_specs <- df$agent_specs - 1
    }
...
Specs <- function(threshold, arrai, df, turn){
  ...
  spec_drift <- abs(df$project_specs - df$agent_specs)
  ...
  if(spec_drift >= threshold){
    ...
    for(i in 1:dim(arrai)[3]){
      arrai[turn, 3,] <- arrai[turn-1, 3,i] - (arrai[turn-1, 3,i]*0.1)
      if(arrai[turn, 3, i] <= 0){
        arrai[turn, 3, i] <- 0
      }
      arrai[turn, 4, i] <- arrai[turn, 3, i]/arrai[turn, 2, i]
...
# testing Simulation 
twodf <- Simulation(100,5,0.75, scrum = FALSE, specs = "changing")
twodf[[2]]
...
- (solve today) I want there to be a way I can see the development of specs over time. 
...
- (quick think after a break - it's okay if ypu don't solve today) ALso rethink why the specs variable needs to go both ways and not just one. 

There are a bunch more variables I would like to be able to adjust withoout re-defining the function.
