...
Simulation <- function(n_turns, n_agent, start_prob, scrum = TRUE, specs = "changing", threshold, len, penalty, no, pos, neg, beta, odds){
  ...
  if(scrum == TRUE){
    ...
    for(i in 1:n_agent){
      eff <- rtruncnorm(1, a=1, b=4, mean = 3, sd = 0.5)
      rew <- rtruncnorm(1, a=1, b=eff, mean = 2, sd = 0.5)
      agents[1,2,i] <- eff
      agents[1,3,i] <- rew
      ...
      for(i in 1:n_agent){
        ...
        if(colab == "sucess"){
          rew <- rtruncnorm(1, a=1, b=4, mean = 3, sd = 1)
          eff <- rtruncnorm(1, a=1, b=rew, mean = 2, sd = 1)
          agents[1,1,i] <- 1
          agents[1,2,i] <- eff
          agents[1,3,i] <- rew
