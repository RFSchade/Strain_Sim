...
```{r}
# Loading packages 
pacman::p_load(pacman, tidyverse, truncnorm, groupdata2)
...
Simulation <- function(n_turns, n_agent, start_prob, scrum = TRUE, specs = "changing", threshold, len, penalty, no, pos, neg, beta, odds, s_beta){
  ...
  for(i in 1:n_agent){
    agents[,6,i] <- rnorm(1, mean=0.01, sd=0.05)
  }
  ...
  if(scrum == TRUE){
    ...
    for(i in 1:n_agent){
      eff <- rtruncnorm(1, a=1, b=1.10, mean = 1.07, sd = 0.01)
      rew <- rtruncnorm(1, a=1, b=eff, mean = 1.04, sd = 0.01)
      agents[1,2,i] <- 1 * eff
      agents[1,3,i] <- 1 * rew
      ...
      for(i in 1:n_agent){
        ...
        if(colab == "sucess"){
          eff <- rtruncnorm(1, a=1, b=1.10, mean = 1.04, sd = 0.01)
          rew <- rtruncnorm(1, a=eff, b=1.10, mean = 1.07, sd = 0.01)
          agents[1,1,i] <- 1
          agents[1,2,i] <- 1 * eff
          agents[1,3,i] <- 1 * rew
          ...
          eff <- rtruncnorm(1, a=1, b=1.10, mean = 1.07, sd = 1)
          rew <- rtruncnorm(1, a=1, b=eff, mean = 1.04, sd = 1)
          agents[1,2,i] <- 1 * eff
          agents[1, 3, i] <- 1 * rew
          agents[1,4,i] <- log(agents[1,2,i]/agents[1,3,i])
          agents[1, 5, i] <- agents[1,6,i] + (beta * agents[1, 4, i] * odds)
...
Specs <- function(threshold, arrai, df, turn, penalty, no, pos, neg, beta, odds){
  ...
  if(spec_drift >= threshold){
    ...
    for(i in 1:dim(arrai)[3]){
      arrai[turn,3,i] <- arrai[turn,3,i] - round((arrai[turn, 3,i]*penalty))
      if(arrai[turn, 3, i] < 1){
        arrai[turn, 3, i] <- 1
      }
      arrai[turn,4,i] <- log(arrai[turn,2,i]/arrai[turn,3,i])
