...
Simulation <- function(n_turns, n_agent, start_prob, scrum = TRUE, specs = "changing", threshold, len, penalty, no, pos, neg, beta, odds){
  ...
  for(i in 1:n_agent){
    agents[,6,i] <- rnorm(1, mean=0.5, sd=0.02)
  }
  ...
  if(scrum == TRUE){
    ...
    for(i in 1:n_agent){
      eff_inc <- rtruncnorm(1, a=1.01, b=1.05, mean = 1.03, sd = 0.01)
      rew_inc <- rtruncnorm(1, a=1.01, b=eff_inc, mean = 1.03, sd = 0.01)
      agents[1,2,i] <- 1 * eff_inc
      agents[1,3,i] <- 1 * rew_inc
      ...
      for(i in 1:n_agent){
        ...
        if(colab == "sucess"){
          rew_inc <- rtruncnorm(1, a=1.01, b=1.05, mean = 1.03, sd = 0.01)
          eff_inc <- rtruncnorm(1, a=1.01, b=rew_inc, mean = 1.03, sd = 0.01)
          agents[1,1,i] <- 1
          agents[1,2,i] <- 1 * eff_inc
          agents[1,3,i] <- 1 * rew_inc
...
Work <- function(colab_prob, arrai, turn, beta, odds) {
  ...
  for(i in 1:dim(arrai)[3]){
    ...
    if(colab == "sucess"){
      ...
      eff_inc <- rtruncnorm(1, a=1.01, b=rew_inc, mean = 1.03, sd = 0.01)
      arrai[turn,1,i] <- 1
      arrai[turn,2,i] <- arrai[turn-1,2,i] * eff_inc
      arrai[turn,3,i] <- arrai[turn-1,3,i] * rew_inc
      ...
      rew_inc <- rtruncnorm(1, a=1.01, b=eff_inc, mean = 1.03, sd = 0.01)
      arrai[turn,2,i] <- arrai[turn-1,2,i] * eff_inc
      arrai[turn,3,i] <- arrai[turn-1,2,i] * rew_inc
      arrai[turn,4,i] <- log(arrai[turn,2,i]/arrai[turn,3,i])
      arrai[turn,5,i] <- arrai[turn,6,i] + (beta * arrai[turn,4,i] * odds)
    }
  }
