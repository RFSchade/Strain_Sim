...
```{r}
# Loading packages 
pacman::p_load(pacman, tidyverse, truncnorm)
...
Simulation <- function(n_turns, n_agent, start_prob, scrum = TRUE, specs = "changing", threshold, len, penalty, no, pos, neg, beta, odds){
  ...
  agents[1, 2,] <- rtruncnorm(1, a=1, b=3, mean = 2, sd = 0.5)
  agents[1, 3,] <- rtruncnorm(1, a=1, b=3, mean = 2, sd = 0.5)
  # Setting baseline
  for(i in 1:n_agent){
  ...
  if(scrum == TRUE){
    ...
    for(i in 1:n_agent){
      agents[1,2,i] <- rtruncnorm(1, a=1, b=3, mean = 2, sd = 0.5)
      agents[1,3,i] <- rtruncnorm(1, a=1, b=agents[1,2,i], mean = 2, sd = 0.5)
      ...
      for(i in 1:n_agent){
        ...
        if(colab == "sucess"){
          agents[1,1,i] <- 1
          agents[1,3,i] <- rtruncnorm(1, a=1, b=3, mean = 2, sd = 0.5)
          agents[1,2,i] <- rtruncnorm(1, a=1, b=agents[1,3,i], mean = 2, sd = 0.5)
...
Work <- function(colab_prob, arrai, turn, beta, odds) {
  ...
  for(i in 1:dim(arrai)[3]){
    ...
    if(colab == "sucess"){
      arrai[turn,1,i] <- 1
      arrai[turn,3,i] <- rtruncnorm(1, a=1, b=3, mean = 2, sd = 0.5)
      arrai[turn,2,i] <- rtruncnorm(1, a=1, b=arrai[turn,3,i], mean = 2, sd = 0.5)
      arrai[turn,4,i] <- log(arrai[turn,2,i]/arrai[turn,3,i])
      arrai[turn,5,i] <- arrai[turn,6,i] + (beta * arrai[turn,4,i] * odds)
    } else {
      arrai[turn,2,i] <- rtruncnorm(1, a=1, b=3, mean = 2, sd = 0.5) 
      arrai[turn,3,i] <- rtruncnorm(1, a=1, b=arrai[turn,2,i], mean = 2, sd = 0.5)
      arrai[turn,4,i] <- log(arrai[turn,2,i]/arrai[turn,3,i])
      arrai[turn,5,i] <- arrai[turn,6,i] + (beta * arrai[turn,4,i] * odds)
    }
  }
