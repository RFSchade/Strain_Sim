...
df_wide_re <- gather(df_re, key = "agent", value = "ratio", agent_1:agent_5)

ggplot(df_wide_re, aes(x = turn, y = ratio, color = agent)) + geom_line(size = 1) + geom_vline(data = df_wide_re %>% select(turn, breakdown) %>%  filter(breakdown == 1), aes(xintercept = turn), alpha = 0.5)
...
Simulation <- function(n_turns, n_agent, start_prob, scrum = TRUE, specs = "changing", threshold, len, penalty, no, pos, neg, beta, odds){
  ...
  agents <- array(0,c(n_turns,5,n_agent))
  dimnames(agents)[[2]] <- c("sucess", "effort", "reward", "ratio", "strain")
  ...
  if(scrum == TRUE){
    ...
    for(i in n_agent){
      ...
      agents[1, 5, i] <- rnorm(1, mean=2, sd=0.2) + (beta * agents[1, 4, i] * odds)
    }
