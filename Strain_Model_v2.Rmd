...
Simulation <- function(n_turns, n_agent, start_prob, scrum = TRUE, specs = "changing", threshold, len, penalty, no, pos, neg, beta, odds, s_beta){
  ...
  for(i in 1:n_agent){
    agents[,6,i] <- rnorm(1, mean=0.5, sd=0.05)
  }
  ...
  if(scrum == TRUE){
    ...
    for(i in 1:n_agent){
      eff <- rtruncnorm(1, a=1, b=10, mean = 7, sd = 1)
      rew <- rtruncnorm(1, a=1, b=eff, mean = 4, sd = 1)
      agents[1,2,i] <- eff
      agents[1,3,i] <- rew
      ...
      for(i in 1:n_agent){
        ...
        if(colab == "sucess"){
          rew <- rtruncnorm(1, a=1, b=10, mean = 7, sd = 1)
          eff <- rtruncnorm(1, a=1, b=rew, mean = 4, sd = 1)
          agents[1,1,i] <- 1
          agents[1,2,i] <- eff
          agents[1,3,i] <- rew
          ...
          eff <- rtruncnorm(1, a=1, b=4, mean = 3, sd = 1)
          rew <- rtruncnorm(1, a=1, b=eff, mean = 2, sd = 1)
          agents[1,2,i] <- eff
          agents[1, 3, i] <- rew
          agents[1,4,i] <- log(agents[1,2,i]/agents[1,3,i])
          agents[1, 5, i] <- agents[1,6,i] + (beta * agents[1, 4, i] * odds)
...
Work <- function(colab_prob, arrai, turn, beta, odds, s_beta) {
  ...
  for(i in 1:dim(arrai)[3]){
    ...
    if(colab == "sucess"){
      rew <- rtruncnorm(1, a=1, b=10, mean = 7, sd = 1)
      eff <- rtruncnorm(1, a=1, b=rew, mean = 5, sd = 1)
      arrai[turn,1,i] <- 1
      arrai[turn,2,i] <- eff
      arrai[turn,3,i] <- rew
      ...
      eff <- rtruncnorm(1, a=1, b=10, mean = 4, sd = 1)
      rew <- rtruncnorm(1, a=1, b=eff, mean = 4, sd = 1)
      arrai[turn,2,i] <- eff
      arrai[turn,3,i] <- rew
      arrai[turn,4,i] <- log(arrai[turn,2,i]/arrai[turn,3,i])
