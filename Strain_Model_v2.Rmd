...
df <- tibble("turn" = 1:1000,"agent_1" = twodf[[2]][, 5, 1], "agent_2" = twodf[[2]][, 5, 2], "agent_3" = twodf[[2]][, 5, 3], "agent_4" = twodf[[2]][, 5, 4], "agent_5" = twodf[[2]][, 5, 5], "breakdown" = as.factor(twodf[[1]][,4]$breakdown))
...
# making dataframe 
df_p <- tibble("turn" = 1:1000,"agent_1" = logit2prob(twodf[[2]][, 5, 1]), "agent_2" = logit2prob(twodf[[2]][, 5, 2]), "agent_3" = logit2prob(twodf[[2]][, 5, 3]), "agent_4" = logit2prob(twodf[[2]][, 5, 4]), "agent_5" = logit2prob(twodf[[2]][, 5, 5]), "breakdown" = as.factor(twodf[[1]][,4]$breakdown))
...
df_r <- tibble("turn" = 1:1000,"agent_1" = twodf[[2]][, 4, 1], "agent_2" = twodf[[2]][, 4, 2], "agent_3" = twodf[[2]][, 4, 3], "agent_4" = twodf[[2]][, 4, 4], "agent_5" = twodf[[2]][, 4, 5], "breakdown" = as.factor(twodf[[1]][,4]$breakdown))
...
df_re <- tibble("turn" = 1:1000,"agent_1" = twodf[[2]][, 3, 1], "agent_2" = twodf[[2]][, 3, 2], "agent_3" = twodf[[2]][, 3, 3], "agent_4" = twodf[[2]][, 3, 4], "agent_5" = twodf[[2]][, 3, 5], "breakdown" = as.factor(twodf[[1]][,4]$breakdown))
...
df_av <- tibble("turn" = 1:1000, "effort" = rowMeans(ef), "reward" = rowMeans(re), "breakdown" = as.factor(twodf[[1]][,4]$breakdown))
...
df_num <- tibble("turn" = 1:1000, "agent_1_re" = twodf[[2]][, 3, 1], "agent_2_re" = twodf[[2]][, 3, 2], "agent_3_re" = twodf[[2]][, 3, 3], "agent_4_re" = twodf[[2]][, 3, 4], "agent_5_re" = twodf[[2]][, 3, 5],"agent_1_ef" = twodf[[2]][, 2, 1], "agent_2_ef" = twodf[[2]][, 2, 2], "agent_3_ef" = twodf[[2]][, 2, 3],  "agent_4_ef" = twodf[[2]][, 2, 4], "agent_5_ef" = twodf[[2]][, 2, 5], "breakdown" = as.factor(twodf[[1]][,4]$breakdown)) %>% mutate("agent_1" = agent_1_ef - agent_1_re, "agent_2" = agent_2_ef - agent_2_re, "agent_3" = agent_3_ef - agent_3_re, "agent_4" = agent_4_ef - agent_4_re, "agent_5" = agent_5_ef - agent_5_re) %>% select(turn, agent_1, agent_2, agent_3, agent_4, agent_5, breakdown)
...
```{r}
Simulation <- function(n_turns, n_agent, start_prob, scrum = TRUE, specs = "changing", threshold, len, penalty, no, pos, neg, beta, odds, s_beta){
  ...
  for(i in 1:n_agent){
    agents[,6,i] <- rnorm(1, mean=0.5670929, sd=0.01)
  }
  ...
  if(scrum == TRUE){
    # first turn in a sprint does is not sucessful
    # Calculating strain and effort/reward
    for(i in 1:n_agent){
      ...
      for(i in 1:n_agent){
        ...
        if(colab == "sucess"){
          ...
          eff <- rtruncnorm(1, a=1.05, b=1.10, mean = 1.07, sd = 0.01)
          rew <- rtruncnorm(1, a=1, b=1.05, mean = 1.04, sd = 0.01)
          agents[1,2,i] <- 1 * eff
          agents[1, 3, i] <- 1 * rew
          agents[1,4,i] <- log(agents[1,2,i]/agents[1,3,i])
          agents[1, 5, i] <- agents[1,6,i] + (beta * agents[1, 4, i] * odds)
...
Work <- function(colab_prob, arrai, turn, beta, odds, s_beta) {
  ...
  for(i in 1:dim(arrai)[3]){
    ...
    s_prob <- colab_prob  + (logit2prob(arrai[turn-1,5,i]) * s_beta)
...
Specs <- function(threshold, arrai, df, turn, penalty, no, pos, neg, beta, odds){
  ...
  if(spec_drift >= threshold){
    ...
    for(i in 1:dim(arrai)[3]){
      arrai[turn,3,i] <- arrai[turn,3,i] - round(abs((arrai[turn, 3,i]*penalty)))
      if(arrai[turn, 3, i] < 1){
        arrai[turn, 3, i] <- 1
      }
      arrai[turn,4,i] <- log(arrai[turn,2,i]/arrai[turn,3,i])
