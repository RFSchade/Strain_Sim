...
Simulation <- function(n_turns, n_agent, start_prob, scrum = TRUE, specs = "changing", threshold, len, penalty, no, pos, neg, beta, odds, s_beta){
  ...
  for(i in 1:n_agent){
    agents[,6,i] <- rnorm(1, mean=0.549834, sd=0.01)
  }
  ...
  if(scrum == TRUE){
    ...
    for(i in 1:n_agent){
      ...
      strain <- agents[1,6,i] + (beta * agents[1, 4, i] * odds)
...
Work <- function(colab_prob, arrai, turn, beta, odds, s_beta) {
  ...
  for(i in 1:dim(arrai)[3]){
    ...
    s_prob <- arrai[turn,7,i] + (logit2prob(arrai[turn-1,5,i]) * s_beta)
    ...
    if(colab == "sucess"){
      ...
      strain <- arrai[turn,6,i] + (beta * arrai[turn,4,i] * odds)
